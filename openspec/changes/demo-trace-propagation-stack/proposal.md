...
- Deploy the upstream **GO Feature Flag (GOFF) relay proxy**, configured with its native Kubernetes ConfigMap retriever so flag definitions come from cluster config rather than an external SaaS. The backend registers the OpenFeature GOFF provider, letting an operator toggle both an application-level flag (`demo-nats-flow`) and `otelnats`'s own `otel-nats-tracing` instrumentation flag by editing a ConfigMap — no restart.
...
- `feature-flag-relay-proxy`: Behavior of the relay proxy service — how it loads/watches flag config from Kubernetes, the evaluation API it exposes, and how the Go backend consumes it to gate a demo feature.
